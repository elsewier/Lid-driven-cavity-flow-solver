...
program L_shaped_cavity_solver
        ...
        ! initialize eacch block 
        call initialize_block(blocks(1), 1) 
        call initialize_block(blocks(2), 2) 

        ! generate the grid for each block 
        call generate_grid(blocks(1))
        call generate_grid(blocks(2))
